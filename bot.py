...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    welcome_text = get_text("🖥 Asosiy menyudasiz")
...
async def earn_money(message: types.Message):
    ...
    if user['level'] >= 1:
        ...
        kb_rows.append([InlineKeyboardButton(text=f"👆 {CURRENCY_NAME} CLICK", callback_data="clicker_process")])
    else:
...
async def service_confirm(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ Buyurtmangiz qabul qilindi!", reply_markup=main_menu(message.from_user.id))
    await state.clear()
